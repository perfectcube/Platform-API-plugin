...
* Description
*/
class RedisLocalPlug extends ProviderPlug{
 	...
 	public function check($options=''){
        ...
        $info['info'] = __('RedisLocal Connection Failed. Check your connection host, port, and password');

        $redis = new Redis();
        $host = parse_url(getenv('REDIS_LOCALHOST'), PHP_URL_HOST);
        $port = parse_url(getenv('REDIS_LOCALHOST'), PHP_URL_PORT);
        $pass = parse_url(getenv('REDIS_LOCALHOST'), PHP_URL_PASS);
        ...
        if ($connected) {
            ...
            if ($have_pass) {
                $info['info'] = __('RedisLocal Login Failed. Check your connection password');
